...
# ------------------ 시뮬레이션 함수 ------------------
def simulate_dual_spread_with_factcheck(graph, agent_types, fake_prob_map, true_prob_map,
                                        seed_count=10, max_steps=20,
                                        factcheck_step=10, factcheck_coverage=0.3, reduced_fake_prob=0.01):
    ...
    for step in range(max_steps):
        ...
        # 팩트체크 또는 필터링 기술 적용 시점
        if step == factcheck_step:
            num_to_reduce = int(NUM_NODES * factcheck_coverage)
            affected = random.sample(range(NUM_NODES), num_to_reduce)
            for i in affected:
                fake_probs[i] = reduced_fake_prob  # 퍼뜨릴 확률 매우 낮아짐
...
history_fake, history_true = simulate_dual_spread_with_factcheck(
    ...
    max_steps=20,
    factcheck_step=10,
    factcheck_coverage=0.3,
    reduced_fake_prob=0.01
)
